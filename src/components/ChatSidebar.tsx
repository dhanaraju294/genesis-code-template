...
import { Search, MessageSquare, Bookmark, User, Clock, Heart, HeartCrack, Building2, ChevronDown, ChevronRight, Menu, Plus } from "lucide-react";
...
const menuItems = [
  ...
  { title: "New Chat", icon: Plus, action: "newChat" },
...
export function ChatSidebar({ collapsed, onToggleSidebar, onNewChat }: ChatSidebarProps) {
              ...
              <SidebarGroupContent>
                <SidebarMenu>
                  {menuItems.map((item, index) => (
                    <SidebarMenuItem key={item.title} className="animate-fade-in mb-1" style={{ animationDelay: `${index * 0.1}s` }}>
                      <SidebarMenuButton 
                        asChild={!item.action}
                        onClick={item.action === "newChat" ? onNewChat : undefined}
                        className="text-sidebar-foreground hover:bg-sidebar-accent transition-all duration-300 hover-scale w-full"
                      >
                        {item.href ? (
